function [Mitral GraProximal GraDistal Feedforward Pyramidal Feedback OSNsource param InputCurrent] = noisy_OB_PC_network_2CG_modified(inputFile,Delay,Wfrac)
...
refracgraprox = 150; % Refractory period after firing (ms)
...
% sum of dendritic membrane voltages for a single GC soma
Vdistsum = zeros(param.nGraprox,1); %
...
countrefracgradist = zeros(param.nGraprox,1); % Refractory period counter. This is assumed to
% be the same for all distal dendrites per GC soma, so it is initialized to
% number of graprox cells
...
tauCA1 = Restgradist; % CA's rising tau.
tauCA2 = Restgradist; % CA's falling tau.
...
for ii = 1:param.nGradist
    ...
    tauCA1(ii) = GraDistal{ii}.tauCA1;
    tauCA2(ii) = GraDistal{ii}.tauCA2;
...
%     EAMPAgraprox(ii) = GraProximal{ii}.EAMPA;
    ...
    VCAthresh(ii) = GraDistal{ii}.VCAthresh;
...
% tCa0dist is a matrix that stores spike times for Ca spikes
tCA0dist = zeros(param.nGraprox,round(param.tsim / param.dt)) - 10000000;
% calculate maxg for Ca spike
t = 0:param.dt:100;
g_range = (exp(- t / tauCA2(1)) - exp(- t / tauCA1(1)));
maxgCAdist = max(g_range);
...
%         Iproxdist_matrix(:,tt) = Iproxdist(:);
        ...
        % Use proximal GC membrane voltage as Resting potential
        % [nGradist x nGraprox] x [nGraprox x 1] = [nGradist x 1]
        Restgradist_mod = gmaxAMPAgradist(1) * MatProxDist * Vgraprox_nospike(:,tt-1);
        Iproxdist_matrix(:,tt) = Restgradist_mod(:);
    ...
    Vgradist(:,tt) = Vgradist(:,tt - 1) + (param.dt ./ taugradist(:)) .* (Rgradist(:) .*...
        ...
        - Vgradist(:,tt - 1) + Restgradist_mod(:) + Vnoisegradist);
    
    % + 0.01*ones(param.nGradist,1)
    
    % "firing probability" for Graded inhibition from distal gra dendrites
    Pfiregradist(:,tt) = NSpikeP(Vgradist(:,tt),Restgradist_mod,Threshgradist);
...
%         Idistprox(:) = Pfiregradist(:,tt)'*MatProxDist .* gmaxAMPAgraprox';
        Vdistsum(:) = (Vgradist(:,tt)'*MatProxDist)/nds; % Avg dendritic membrane potentials per soma at tt-1
        I = find(Vdistsum >= VCAthresh(1));
        
        J = find(countrefracgradist == 0);
        
        K = intersect(I,J);
        
        % Reset t only for those neurons that spiked outside of refractory period
        if ~isempty(K)
            tCA0dist(K,ceil(t/param.dt):end) = t;
            countrefracgradist(K) = refracgradist / param.dt; % neurons that just fired get
        % into the refractory period
        end
        g = (exp(-(t - tCA0dist(:,tt)) / tauCA2(1)) - exp(-(t - tCA0dist(:,tt)) / tauCA1(1))) / maxgCAdist;

        Idistprox(:) = gmaxAMPAgraprox(1) * g;
%         Idistprox(:) = Idistprox(:)./Rgraprox(:);
        Idistprox_matrix(:,tt) = Idistprox(:);
        
        
        % move one time step through Ca spike refractory period
        I = countrefracgradist > 0;
        countrefracgradist(I) = countrefracgradist(I) - 1;
...
%     
        ...
        tLLD0graprox(I) = t;
        J = (t - tLLD0graprox) < tauLLDgraprox(1); % index cells that are still in the LLD period
        VLLDgraprox(:,tt) = zeros(param.nGraprox,1);
        VLLDgraprox(J,tt) = ELLDgraprox(1) .* (1 - ((t - tLLD0graprox(J))/tauLLDgraprox(1))); % linearly decreasing LLD
    end
